export const strToFloat = (text: string | undefined): number => {
  try {
    if (text) {
      ...
      if (formatText) {
        return parseFloat(formatText.join(''))
      }
    }
  } catch (e) {
...
export const strTimeToSecond = (time: string): number => {
  ...
  keys.map((key) => {
    ...
    second += mapKey[<keyof typeof mapKey>letter] * parseInt(number)
  })

  return second
}

export const getStringBeforeRegex = (text: string, match: string, regex = '.+?'): string => {
