...
} from '@main/integrations/ipc/interfaces';
...
export class ElectronModule {
  ...
  private static getProviderName(name: unknown) {
    ...
    if (
      ...
      if (ElectronModule.isMainNameUsed) {
        throw new Error(
          `Default name "${ELECTRON_WINDOW_DEFAULT_NAME}" has been used, please specify a name for the window`,
        );
      }
  ...
  static registerAsync(options: ElectronModuleAsyncOptions): DynamicModule {
    ...
    if (isElectron) {
      const nameKey = Array.isArray(options.name)
        ? options.name.join(',')
        : options.name;
      ...
      for (let i = 0; i < names.length; i++) {
        ...
        providers.push({
          ...
          useFactory: (value: ElectronModuleProviderValue) => {
            if (Array.isArray(value)) return value[i] || null;

            if (isSingleBind) return null;

            isSingleBind = true;
            return typeof value === 'object' && 'win' in value
              ? value.win
              : value;
          },
